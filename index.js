...
async function start() {
  try {
    ...
    const Matrix = makeWASocket({
      ...
      browser: ["SPIRITY-XMD", "Chrome", "1.0.0"],
      auth: state,
      getMessage: async (key) => {
        ...
        return { conversation: "SPIRITY-XMD whatsapp user bot" };
      },
    });
      ...
      if (connection === "open") {
        ...
        if (!hasSentStartMessage) {
          const firstMessage = [
            `◈━━━━━━━━━━━━━━━━◈`,
            `│❒ *${getGreeting()}*`,
            `│❒ Welcome to *SPIRITY-XMD*! You're now connected.`,
            ``,
            `✨ *Bot Name*: SPIRITY-XMD`,
...
app.get("/", (req, res) => {
  res.send("SPIRITY-XMD is running!");
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`SPIRITY-XMD server running on port ${PORT}`);
});
